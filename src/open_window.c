...
sfVector2f **create_2d_map(int **map, int width, int height)
{
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            sfVector2f projected = project_iso_point(x * TILE_SIZE, y * TILE_SIZE, map[y][x] * 10);
...
sfVertexArray *create_textured_quad(sfVector2f p1, sfVector2f p2, sfVector2f p3, sfVector2f p4, sfTexture *texture)
{
...
int draw_2d_map(sfRenderWindow *window, sfVector2f **map_2d, int width, int height, sfTexture *texture)
{
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            if (x < width - 1) { // Dessiner les lignes horizontales
            ...
            if (y < height - 1) { // Dessiner les lignes verticales
            ...
            if (x < width - 1 && y < height - 1) { // Dessiner les quads texturés
                sfVertexArray *quad = create_textured_quad(
                    map_2d[y][x], map_2d[y][x + 1],
                    map_2d[y + 1][x + 1], map_2d[y + 1][x],
                    texture);
                if (quad) {
...
static void analyse_events(sfRenderWindow *win, sfEvent *event, sfVector2f **my_2d_map)
{
...
void while_window_open(sfRenderWindow *win, sfEvent event, sfVector2f **my_2d_map, sfTexture *texture)
{
    while (sfRenderWindow_pollEvent(win, &event))
        analyse_events(win, &event, my_2d_map);
    draw_2d_map(win, my_2d_map, SIZE_OF_MAP, SIZE_OF_MAP, texture);
    sfRenderWindow_display(win);
}

int **create_map_rand(void)
{
...
{
    sfRenderWindow *win = sfRenderWindow_create((sfVideoMode){WINDOW_WIDTH, WINDOW_HEIGHT, 32},
        "My_worldbootstrap", sfDefaultStyle, NULL);
    sfEvent event;
